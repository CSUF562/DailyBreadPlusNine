...
function createPainter({canvas, spreadEl, responsiveEl, lineWeightEl, lineOpacityEl, undoEl, redoEl, clearEl}){
  ...
  function palette(m){
    ...
    if(m.flow>.50)return[168,188,208];
    return[202,220,242];
  }

  function dab(x,y,r,m,a,seed,s=spread()){
    const [h1,h2,h3]=palette(m);
    const R=r*s;
    const g=ctx.createRadialGradient(x,y,0,x,y,R);
    g.addColorStop(0,`hsla(${h1},62%,56%,${a*1.25})`);
    g.addColorStop(.42,`hsla(${h2},55%,62%,${a})`);
    g.addColorStop(.74,`hsla(${h3},48%,68%,${a*.5})`);
    g.addColorStop(1,`hsla(${h1},40%,72%,0)`);
    ctx.fillStyle=g;
    ctx.beginPath();
    ...
    if(seed%9===0){
      ctx.fillStyle=`hsla(${h2},55%,58%,${a*.45})`;
      ctx.beginPath();
      ctx.arc(
        x+Math.sin(seed)*r*.15*s,
        y+Math.cos(seed)*r*.15*s,
        r*.45*s,0,Math.PI*2
      );
      ctx.fill();
    }
